...
func Paging(defaultLimit int64) func(next http.Handler) http.Handler {
...
func pageParameters(r *http.Request, w http.ResponseWriter, defaultLimit int64) (paging.Parameters, error) {
	...
	if cursor == "" {
		return paging.NewParameters(int64(limitInt), nil, w), nil
	} else {
		...
		return paging.NewParameters(int64(limitInt), &dcursor, w), nil
	}
